...
hardware provide many more theoretical FLOPS than the x86 processor packages
with which they're commonly coupled.
...
It is, however, almost certainly safer. GPUs have traditionally been used to
accelerate games and CAD (single-instance, interactive tasks) and batch
rendering (decomposable, compute-bound tasks, for which a deinterleaved
schedule is optimal). Facilities supporting multiple threads or processes are
thus primitive, especially at the hardware level. When the GPU was limited to
...
are the precise applications driving GPGPU. We provide a system-centric threat
...
base. We close with software-level defenses against our attacks.
