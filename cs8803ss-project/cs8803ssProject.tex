...
blitting, fogging and aliasing framebuffers, its minimal isolation and
assurance capabilities were likely tolerable. Compromises of scientific data,
cryptographic materials and SCADA systems are rather more serious, yet these
are the precise applications driving the adoption of GPGPU. We provide a system-centric threat
model analysis of NVIDIA's CUDA system, and attack it. Our results suggest that,
unless GPGPU systems add complexity related to security state (and thus devote
less transistor budget to performance), they cannot provide a trusted computing
base. We close with software-level defenses against our attacks, and their limitations.
...
Well-known threat models include the ``CIA Triad'' (extended by the
``Parkerian Hexad'') and Microsoft's STRIDE. The latter, developed as part of
...
\section{Conclusions} 
The Saltzer-Schroeder Practices, recommendations of the Orange Book, and
requirements of the Common Criteria have long provided general principles for
...
\begin{itemize}
\item Increasing the clock frequency exacerbates cooling and power problems
(dynamic power consumption is proportional to frequency). Furthermore,
clock frequency increases frequently require decomposition of pipeline
stages, thus exacerbating branch misprediction delays and pressuring any OOO
system.
...
to the profound limitations of instruction level parallelism. Like improvements
...
instance, feeding uops from Nehalem's Loop Stream Detector results in power-down
