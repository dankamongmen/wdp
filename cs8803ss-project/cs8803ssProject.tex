...
\subsection{Tools}
\texttt{strace}\cite{stracecode} was used to track system calls of various CUDA
binaries. \texttt{nvtrace}, a Valgrind plugin from the Nouveau
Project\cite{nvtrace}, was used to assist in deciphering the \texttt{ioctl(2)}s
issued to the NVIDIA proprietary driver. The Linux kernel's MMIOTrace\cite{mmiotrace}
infrastructure tracked memory-mapped I/O operations at a deep level.
