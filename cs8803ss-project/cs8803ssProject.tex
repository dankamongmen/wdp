...
CUDA is accessed via \texttt{open(2)}ing the device node and invoking \texttt{ioctl(2)}
upon the resulting file descriptor. All accesses to the card and its memory
...
and that these file descriptors can (by design or oversight) be passed via
...
memory (and vice versa) to be shared among computation devices.
