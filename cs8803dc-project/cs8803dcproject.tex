...
fully predicated instruction set, such as ARM or IA64. This transformation is
...
Unoptimized code can of course be easily pruned, but we assume reasonable
optimization for space---in which case we are likely to achieve much
reductions---or speed, which has as its most basic heuristic ``minimize dynamic
...
were demonstrated to be innocuous. Sets of \texttt{pusha}/\texttt{popa}, used
to minimize the number of stack writes, could be replaced with a single
\texttt{pusha}/texttt{popa} pair. This is unlikely to affect performance negatively,
